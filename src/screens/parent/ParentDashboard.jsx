...
// Filter fees  only show current and past months (no future)

const filterPastAndCurrentFees = (fees = []) => {

  const now = new Date();

  const currentMonth = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;

  return fees.filter(f => f.month <= currentMonth);
...
const ChildDetailModal = ({ child, onClose }) => {
  ...
  const rawFees = child.fees;

  const fees = {

    ...rawFees,

    unpaidFees: filterPastAndCurrentFees(rawFees?.unpaidFees),

    unpaidCount: filterPastAndCurrentFees(rawFees?.unpaidFees).length,

    totalOutstanding: filterPastAndCurrentFees(rawFees?.unpaidFees)
          ...
          <View style={dStyles.section}>
            ...
            <View style={dStyles.feeCards}>
              ...
              <View style={[dStyles.feeCard, { borderColor: '#F59E0B' }]}>
                ...
                <Text style={[dStyles.feeCardValue, { color: '#F59E0B' }]}>{fees?.unpaidCount || 0} months</Text>
            ...
            {fees?.unpaidFees?.length > 0 && (
              ...
              <View style={dStyles.unpaidList}>
                ...
                <Text style={dStyles.unpaidTitle}> Unpaid Fees:</Text>
          ...
          <View style={dStyles.section}>
            ...
            {attendance.length === 0 ? (
              ...
              attendance.map((cls, i) => {
                ...
                const pct = typeof cls.percentage === 'string' ? parseInt(cls.percentage) : cls.percentage || 0;
                ...
                return (
                  ...
                  <View key={i} style={dStyles.attCard}>
                    ...
                    {cls.totalSessions > 0 && (
                      ...
                      <>
                        ...
                        <View style={dStyles.attBar}>
                          ...
                          <View style={[dStyles.attFill, { width: `${pct}%`, backgroundColor: cls.atRisk ? '#EF4444' : '#10B981' }]} />
                    ...
                    {cls.atRisk && <Text style={dStyles.attWarning}> Below 80% minimum!</Text>}
              ...
              {recentPayments.map((payment, i) => (
                ...
                <View key={i} style={dStyles.paymentRow}>
                  ...
                  <View style={dStyles.paymentInfo}>
                    ...
                    <Text style={dStyles.paymentDate}>{new Date(payment.paidAt).toLocaleDateString('en-GB')}  {payment.method}</Text>
...
const dStyles = StyleSheet.create({
  ...
  unpaidList: { backgroundColor: '#FFF9F9', borderRadius: 10, padding: 10, borderWidth: 1, borderColor: '#FEE2E2' },

  unpaidTitle: { fontSize: 13, fontWeight: '700', color: '#EF4444', marginBottom: 8 },

  unpaidRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: '#FEE2E2' },
  ...
  attCard: { backgroundColor: '#FAFAFA', borderRadius: 12, padding: 12, marginBottom: 8, borderWidth: 1, borderColor: '#F0F0F0' },
  ...
  paymentRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },

  paymentIcon: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#F0FBF7', alignItems: 'center', justifyContent: 'center' },
...
const ParentDashboard = () => {
  ...
  const atRiskChildren = children.filter(c => c.attendance?.some(a => a.atRisk));
          ...
          <View style={styles.section}>
            ...
            {isLoading ? (
              ...
              children.map((child, i) => {
                ...
                unpaidFees: filterPastAndCurrentFees(child.fees?.unpaidFees),

                totalOutstanding: filterPastAndCurrentFees(child.fees?.unpaidFees)
                  ...
                  >
                    ...
                    <View style={styles.childHeader}>
                      ...
                      <View style={styles.childInfo}>
                        ...
                        <Text style={styles.childDetail}>{student?.admissionNumber}  {student?.grade}</Text>
                    ...
                    {fees?.totalOutstanding > 0 && (
                      ...
                      <View style={styles.feeBanner}>
                        ...
                        <Text style={styles.feeText}>Rs. {fees.totalOutstanding.toLocaleString()} outstanding</Text>
...
const styles = StyleSheet.create({
  ...
  statCard: { flex: 1, backgroundColor: COLORS.white, borderRadius: 16, padding: 14, alignItems: 'center', elevation: 2 },
  ...
  childCard: { backgroundColor: COLORS.white, borderRadius: 16, padding: 16, marginBottom: 12, elevation: 2 },
  ...
  alertBanner: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#FFF9E6', borderRadius: 8, padding: 8, marginBottom: 4, borderWidth: 1, borderColor: '#FDE68A' },

  alertText: { fontSize: 12, color: '#92400E', fontWeight: '600' },

  feeBanner: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#FEF2F2', borderRadius: 8, padding: 8, marginBottom: 4, borderWidth: 1, borderColor: '#FEE2E2' },
  ...
  tipsCard: { backgroundColor: COLORS.white, borderRadius: 16, padding: 16, marginHorizontal: 20, marginBottom: 16, elevation: 2 },
